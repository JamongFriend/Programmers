 ...
 * 풀이 전략:
 * 첫 공격 데미지만큼 체력 감소
 * 치료가능 시간: 몬스터가 공격을 위해 기다리는 시간 - 이전 공격 시간 - 1
 * 연속으로 붕대감기 시전시 치료시간 만큼 회복
 * 연속 시전시 추가 회복
 * 치료가능 시간이 끝나면 데미지 만큼 체력 감소
 * 만약 helath가 0보다 작아지면 -1 반환
 *
 */
...
class Solution {
    public int solution(int[] bandage, int health, int[][] attacks) {
        ...
        for(int i=1; i< attacks.length; i++){
            ...
            int restTime = curTime - lastAttackTime - 1;

            if(restTime > 0){
                health += bandage[1] * restTime;
                if (restTime >= bandage[0]) {
                    health += bandage[2] * (restTime / bandage[0]);
                }
