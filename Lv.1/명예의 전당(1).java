 ...
 *
 * 풀이 전략:
 * 스코어 값을 myeong리스트에 추가
 * myeong의 크기가 k값을 넘으면 가장 작은 값을 제거
 * 스코어 값이 리스트에 추가될 때 마다 리스트의 최소값을 answer배열에 추가
 *
 */
...
class Solution {
    public int[] solution(int k, int[] score) {
        ...
        List<Integer> myeong = new ArrayList<>();

        for(int i = 0; i < score.length; i++){
            myeong.add(score[i]);

            if(myeong.size() > k){
                myeong.remove(Collections.min(myeong));
            }

            answer[i] = Collections.min(myeong);
        }
        return answer;
    }
}